...
// ------------------------- * Palette * -------------------------
void File_LoadPalette(void)
{ // FIXME: Rewrite!
	FILE *fp;

	fp=fopen(BASEDIR"palette32.raw", "rb");
	fread(WolfPal, 1, 256 * 3, fp);
	fclose(fp);
}
...
int File_PML_OpenPageFile(char *filename)
{
  ...
  fread(&PageFile.ChunksInFile, 1, sizeof(unsigned short), fp);
  fread(&PageFile.SpriteStart,	1, sizeof(unsigned short), fp);
  fread(&PageFile.SoundStart,		1, sizeof(unsigned short), fp);
  ...
  PageFile.Pages=(chunk *)calloc(PageFile.ChunksInFile, sizeof(chunk));
	...
	{
		fread(&PageFile.Pages[i].offset, 1, sizeof(long), fp);
	}
...
int File_PML_LoadSprite(int n, unsigned char *block)
{
  ...
  shape_t        *shape;
	...
	if(n<PageFile.SpriteStart || n>=PageFile.SoundStart)
	{
		Con_Printf("FileIO: Sprite index (%d) out of bounds (%d-%d)\n", n, PageFile.SpriteStart, PageFile.SoundStart);
		return 0;
	}
...
// prepare array!
	for(x=0; x<64; x++)
	{
		for(y=0; y<64; y++)
		{
			block[(y*64+x)*4+0]=115;	//	red
			block[(y*64+x)*4+1]=113;	//	green
			block[(y*64+x)*4+2]=115;	//	blue
			block[(y*64+x)*4+3]=0;		//	alpha
		}
	}
	
// Very wired code by Mr. Bruce Lewis (btw creator of DoomGL) need to be rewriten!
	shape=(shape_t *)sprite;

	i=4+( ( shape->rightpix-shape->leftpix+1 )*2);
	...
	for(x=shape->leftpix; x<=shape->rightpix; x++)
  {
    ...
    while(sprdata[idx])
    {
      ...
      {
 				block[(y*64+x)*4+0]=WolfPal[sprite[i]].r;	//	red
				block[(y*64+x)*4+1]=WolfPal[sprite[i]].g;	//	green
				block[(y*64+x)*4+2]=WolfPal[sprite[i]].b;	//	blue
				block[(y*64+x)*4+3]=255;									//	alpha
        i++;
	    }
      idx+=3;
    }
...
// Writes data to file (The filename will be prefixed by the current game directory)
void File_COM_Write(char *filename, void *data, int len)
{
	FILE *fp;
	char name[MAX_OSPATH];
	
	sprintf(name, BASEDIR"%s", filename);
	...
	Con_Printf("FileIO: Write: %s\n", name);
	fwrite(data, 1, len, fp);
	fclose(fp);
}

// malloc & read all data!
unsigned char *File_COM_LoadFile(char *filename)
{
	FILE *fp;
	char name[MAX_OSPATH];
	int len;
...
// FIXME: make a file system!: look for it in the filesystem or pack files
	sprintf(name, BASEDIR"%s", filename);
	len=File_COM_FileOpenRead(name, &fp);
	if(len==-1) return NULL;
	
	buf=malloc(len+1);
	if(!buf)
	{
		Con_Printf("FileIO: Not enough memory for %s, length=%d\n", name, len);
		return NULL;
	}
	fread(buf, 1, len, fp);
	fclose(fp);
	buf[len]=0; // for exec command!
...
/*
	Planes:
	0 - only Alpha (0xFF, 0xFF, 0xFF, data0)
	1 - Gray Scale (data0, data0, data0)
	2 - Gray + A   (data0, data0, data0, data1)
	3 - RGB        (data0, data1, data2)
	4 - RGBA       (data0, data1, data2, data3)
*/
int File_RAW_Read(char *filename, long offset, unsigned char *data, int width, int height, char planes)
{
	FILE *fp;
	long size;
	int n;
	...
	if(fp==NULL)
	{
		Con_Printf("FileIO: File read error: %s\n", filename);
		return 0;
	}
	...
	case 1:
		for(n=0; n<size; n++)
		{
			data[(n<<2)+0]=data[(n<<2)+1]=data[(n<<2)+2]=getc(fp);
		}
		break;
	case 2:
		for(n=0; n<size; n++)
	...
	case 3:
	case 4:
		fread(data, 1, size*planes, fp);
		break;
	default:
		Con_Printf("FileIO: Unknown planes: %d\n", planes);
		fclose(fp);
		return 0;
	}
	fclose(fp);
...
*/
int File_TGA_Read(char *filename, unsigned char *data)
{
	FILE *fp;
	unsigned char header[18];
	int width, height, alpha, x, y;

	fp=fopen(filename, "rb");
	if(fp==NULL)
		{Con_Printf("FileIO: File read error: %s\n", filename); return 0;}
	fread(header, 1, 18, fp);
	// FIXME: add support for all TGA formats!
	if(header[2]!=2)
	{
		Con_Printf("FileIO: Only TGA files of type 2 (uncompressed RGB(A)) are supported\n");
		fclose(fp);
		return 0;
	}
	if(header[1]!=0 || (header[16]!=32 && header[16]!=24)) 
	{
		Con_Printf("FileIO: Only 24 or 32 bpp TGA files are supported\n");
		fclose(fp);
		return 0;
	}
	width=*((unsigned short *)(&header[12]));
	height=*((unsigned short *)(&header[14])); // cool cast, no?
	alpha=(header[16]==32);
	fseek(fp, header[0], SEEK_CUR); // skip comment

	for(y=height-1; y>=0; y--)
		for(x=0; x<width; x++)
		{
			data[(y*width+x)*4+2]=fgetc(fp);
			data[(y*width+x)*4+1]=fgetc(fp);
			data[(y*width+x)*4+0]=fgetc(fp);
			data[(y*width+x)*4+3]=alpha ? fgetc(fp) : 0xFF;
		}
...
void File_CRC_f(void)
{
	if(Cmd_Argc()!=2)
		...
		Con_Printf("CRC-16 CCIT of %s: %d\n", Cmd_Argv(1), File_COM_CRCfile(va(BASEDIR"%s", Cmd_Argv(1))));
}

// ------------------------- * Common GFX Loader * -------------------------

#define PROJECT_DOOR_COUNT 8
// loads texture defined by name, mallocs buffer returns 1 if Ok, 0 if not.
int File_LoadTexture(int name, unsigned char **data, cache_tex *tex)
{
	if(name&TEX_SPRITE_BIT)
	{ // load sprite
		name=SPR_REMAP[(name&(TEX_SPRITE_BIT-1))]+PageFile.SpriteStart;
		*data=malloc(64*64*4);
		if(!File_PML_LoadSprite(name, *data))
			{free(*data); return 0;}
		tex->width=tex->height=64;
		tex->alpha=true;
	}
	else if(name&TEX_DOOR_BIT)
	{ // load door
		name=(name&(TEX_DOOR_BIT-1))+(PageFile.SpriteStart-PROJECT_DOOR_COUNT);
		*data=malloc(64*64*3);
		if(!File_PML_LoadWall(name, *data))
			{free(*data); return 0;}
		tex->width=tex->height=64;
		tex->alpha=false;
	}

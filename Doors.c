...
int Door_SpawnDoor(int x, int y, int type)
{
	...
	case 0x5A:
		...
		Doors[totaldoors].vertical=1;
		break;
	case 0x5B:
		Doors[totaldoors].type=DOOR_HORIZ;
		Doors[totaldoors].vertical=0;
		break;
	case 0x5C:
	...
	for(n=0; n<totaldoors; n++)
	{
		...
		case dr_opening:
			...
			else // opening!
			{
				...
				{ // door is just starting to open, so connect the areas
					...
					if(areabyplayer[Doors[n].area1])
						PlaySoundLocTile(OPENDOORSND, Doors[n].x, Doors[n].y);
				}
		...
		case dr_closing:
			...
			else // closing!
			{
				...
				if(areabyplayer[Doors[n].area1])
				{ // Door Closing sound!
					PlaySoundLocTile(CLOSEDOORSND, Doors[n].x, Doors[n].y);
				}
				Doors[n].tickcount-=t_tk;

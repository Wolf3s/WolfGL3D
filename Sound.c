...
#define MAX_CHANNELS 32 // Limit is 64 
...
id_snd_fmt=
{
	...
	1,								// Mono
	7168,							// Speed (may be 6896 (Wolf DX project) ? ) or just 7000 (I think so)
	7168,							// nSamplesPerSec*(wBitsPerSample/8)*nChannels,
...
// ------------------------- * Devider * -------------------------
void SD_PlaySound(soundnames SoundNum)
{
	int Chn;

	if(!sound_on) return;

	if( (Chn=SD_ChooseChannel(SoundNum))==-1 ) return;
...
void SD_PlaySound3D(soundnames SoundNum, int x, int y)
{
	int Chn;
...
void SD_PlayMusic(int song, int force_restart)
{
	song=MUS_REMAP[song];
	last_song=song; // save last son requested!
...
void SD_PlayImf_f(void)
{
	...
	fname=va(BASEDIR"music\\%s", Cmd_Argv(1));
	len=File_COM_FileOpenRead(fname, &fp);
	if(len==-1)
	{
...
*/
	...
	if(SoundNum==ATKKNIFESND || SoundNum==ATKPISTOLSND || SoundNum==ATKMACHINEGUNSND || SoundNum==ATKGATLINGSND)
	{ // Player is Fireing (channel 0) Player is V.I.P. in the game :)<=>-
		if(Channels[0].Sound!=SoundNum)
		{// Load Sound
			if(!SD_LoadIdSound(SoundNum, 0)) return -1;
		}
		return 0;
	}
...
// translate point to view centered coordinates
	gx-=Player.x; // FIX: Take ViewPoint not Player Pos (may be different!)
	gy-=Player.y;
	gy=-gy; // little hack

// calculate newx
	xt=(int)(gx * CosTable[Player.angle]);
	yt=(int)(gy * SinTable[Player.angle]);
	x =(xt+yt)>>(TILESHIFT);

// calculate newy
	xt=(int)(gx*SinTable[Player.angle]);
	yt=(int)(gy*CosTable[Player.angle]);
	y =(yt+xt)>>(TILESHIFT);
...
void SD_UnCacheSnd(int name)
{
	...
	memmove(snd, snd+1, (int)&cachesnds[--numcachesnds]-(int)snd);
}

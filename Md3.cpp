...
bool CModelMD3::LoadModel(const LPSTR strPath, const  LPSTR strModel, const  LPSTR Skin)
{
	char str_Model[255]  = {0};	// This stores the file name for the head.md3 model
	char strSkin[255]  = {0};	// This stores the file name for the lower.md3 skin
	CLoadMD3 loadMd3;				// This object allows us to load each .md3 and .skin file
	int i, wrongSkins=0;
	char SkinNames[8][128];
	...
	for (i=0; i<3; i++){
		...
		switch (i){
			case 2:sprintf(str_Model,  "%s\\%s_%s.md3",  strPath, strModel,"hi");break;
			case 1:sprintf(str_Model,  "%s\\%s_%s.md3",  strPath, strModel,"mid");break;
			case 0:sprintf(str_Model,  "%s\\%s_%s.md3",  strPath, strModel,"low");
		//loadMd3.ImportMD3(&m_skins[0], str_Model);
		}
		//+Detailzation
	
		// Load the mesh (*_lower.md3) and make sure it loaded properly
		if(!loadMd3.ImportMD3(&m_model[i], str_Model)){
			// Display an error message telling us the file could not be found
			sprintf(str_Model, "Unable to load the model:", str_Model);
			MessageBox(0, str_Model, "Error", MB_OK);
			return false;}}
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////
	Skins=0;
	sprintf(strSkin,  "%s\\%s.skins",  strPath, strModel);
	...
	if(!fin.fail()){while( fin >> strWord){
		...
		if(!isdigit(strWord[0])){// Store the rest of this line and go to the next one
			getline(fin, strLine);
			continue;}
		//up to 8 skins for each model
		fin >> SkinNames[Skins];Skins++;if(Skins>7)break;}
		fin.close();}
	...
	for (i=0; i<Skins; i++){
		...
		m_skins[i].pMaterials=m_model[0].pMaterials;
		m_skins[i].numOfObjects=m_model[0].numOfObjects;
		m_skins[i].numOfMaterials=m_model[0].numOfMaterials;
		m_skins[i].pObject=m_model[0].pObject;

		// Get the skin file names with their path
		//MessageBox(0, SkinNames[i], "Skin", MB_OK);
		sprintf(strSkin,  "%s\\%s%s.skin",  strPath, strModel, SkinNames[i]);

		//MessageBox(0, strSkin, "Skin", MB_OK);
		
		// Load the skin and make sure it loaded properly
		if(!loadMd3.LoadSkin(&m_skins[i-wrongSkins], strSkin)){
			wrongSkins++; continue;}//try the next skin

		// Load the textures
		LoadModelTextures(&m_skins[i],  strPath);}

		Skins-=wrongSkins;//how many skins we have loaded
		if(!Skins){//Display an error message telling us any skin could not be found
			sprintf(strSkin,  "Unable to load any skin for: %s", strModel);
			MessageBox(0, strSkin, "Error", MB_OK);
			return false;}
...
//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////
	...
	{
		// Display an error message telling us the file could not be found
		MessageBox(0, "Unable to load the Animation Config File!", "Error", MB_OK);
		return false;
	}
...
bool CModelMD3::LoadAnimations(LPSTR strConfigFile)
{
	...
	if( fin.fail() )
	{
		...
		MessageBox(0, "Unable to find the Animation Config", "Error", MB_OK);
		return false;
	}
	...
	while( fin >> strWord)
	{
		...
		*/
  			m_model[2].pAnimations.push_back(animations[currentAnim]);//high-res is default
	...
	for(int i=0; i<3; i++)
		m_model[i].numOfAnimations = m_model[i].pAnimations.size();
...
void CModelMD3::UpdateModel(t3DModel *pModel)
{
	...
	if(pModel->numOfAnimations){
		...
		loopingFrames   = pAnim->loopingFrames;}
	...
	if (loopingFrames)
		...
		else if ((pModel->currentFrame + 1) % endFrame != 0)
			pModel->nextFrame = (pModel->currentFrame + 1) % endFrame;
...
void CModelMD3::DrawModel(const int LOD, const int skin)
{
	// Rotate the model to compensate for the z up orientation that the model was saved
	glRotatef(-90, 1, 0, 0);

	//if(m_model[LOD].pObject.size() <= 0) MessageBox(0, "Model[0] is empty!", "??", MB_OK);

	switch	(D) {//added by Idot 
		//{temporary - next vesrion should work with angles}
			case 0:	glTranslatef(X,Y,Z);
					break;
			case 1: glRotatef(180, 0, 0, 1);
					glTranslatef(-X,-Y,Z);
					break;
...
//	UpdateModel(&m_model[0],&m_model[1],&m_model[2]);///////////temporary
	UpdateModel(&m_model[0]);///////////temporary
	UpdateModel(&m_model[1]);///////////temporary
	UpdateModel(&m_model[2]);///////////temporary
//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////
...
//	DrawLink(&m_Lower);
	if (skin<Skins) 
		DrawLink(&m_model[LOD],&m_skins[skin]);
		else DrawLink(&m_model[LOD],&m_skins[Skins-1]);//use the last skin
	//We should restore our system coordinate
	switch	(D) {//added by Idot 
		//{temporary - next vesrion should work with angles}
			case 0:	glTranslatef(-X,-Y,-Z);
					break;
			case 1: glTranslatef(X,Y,-Z);
					glRotatef(-180, 0, 0, 1);
					break;
			case 2: glTranslatef(Y,-X,-Z);
					glRotatef(90, 0, 0, 1);
					break;
			case 3: glTranslatef(-Y,X,-Z);
					glRotatef(-90, 0, 0, 1);
...
void CModelMD3::DrawLink(t3DModel *pModel, t3DModel *pSkin)
{
	// Draw the current model passed in (Initially the legs)
	RenderModel(pModel,pSkin);
...
void CModelMD3::RenderModel(t3DModel *pModel, t3DModel *pSkin)
{
	...
	for(int i = 0; i < pModel->numOfObjects; i++)
	{
		...
		t3DObject *pObject = &pModel->pObject[i];
		t3DObject *pObject_ = &pSkin->pObject[i];
...
//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////
		...
		//if(false)//Fix!!!!!!!!!!
		{
			...
			//int textureID = pSkin->pMaterials[pObject->materialID].texureId;
			int textureID = pSkin->pMaterials[pObject_->materialID].texureId;
		...
		int index;//moved by Idot
		if (true){//optimized
			int fits;
			int fited[2],fited_[2];
			int Strip_Vertex=0;
			fited_[0]=0;fited_[1]=1;
			glBegin(GL_TRIANGLE_STRIP);//optimized by Idot
			//glBegin(GL_TRIANGLES);//GL_TRIANGLE_STRIP
				// Go through all of the faces (polygons) of the object and draw them
				for(int j = 0; j < pObject->numOfFaces; j++)
				{	
					fits=0;
					if (j>0)for(int Vertex_ = 0; Vertex_ < 3; Vertex_++){//WolfGl3D_opt2
							//if (Vertex_==Strip_Vertex) continue;
							if ((Vertex_==fited_[0])||(Vertex_==fited_[1])) continue;
							for(int Vertex_new = 0; Vertex_new < 3; Vertex_new++)
								if (pObject->pFaces[j-1].vertIndex[Vertex_]==
									pObject->pFaces[j].vertIndex[Vertex_new]){
									fits++;
									if (fits<3)	fited[fits]=Vertex_new;
							}		}
	
					if (fits>1){
						fited_[0]=fited[0];fited_[1]=fited[1];
						/*int index;
						for(index=0;index<3;index++)
							if ((index!=fited[0])&&(index!=fited[1])) break;(*/
						if ((fited[0]==0)&&(fited[1]==1)||(fited[0]==1)&&(fited[1]==0))Strip_Vertex=2;
						if ((fited[0]==1)&&(fited[1]==2)||(fited[0]==2)&&(fited[1]==1))Strip_Vertex=1;
						if ((fited[0]==0)&&(fited[1]==2)||(fited[0]==2)&&(fited[1]==0))Strip_Vertex=0; 

						index = pObject->pFaces[j].vertIndex[Strip_Vertex];
									
						// Make sure there is texture coordinates for this (%99.9 likelyhood)
					/*
					if ((j>0)&&
						(((pObject->pFaces[j-1].vertIndex[1]==pObject->pFaces[j].vertIndex[0])&&//WolfGl3D_opt
						(pObject->pFaces[j-1].vertIndex[2]==pObject->pFaces[j].vertIndex[1]))||
						((pObject->pFaces[j-1].vertIndex[1]==pObject->pFaces[j].vertIndex[0])&&
						(pObject->pFaces[j-1].vertIndex[2]==pObject->pFaces[j].vertIndex[1])))
						){
						int index = pObject->pFaces[j].vertIndex[2];*/
						...
						glVertex3f((vPoint1.x + pModel->t * (vPoint2.x - vPoint1.x))/ScaleX,
								   (vPoint1.y + pModel->t * (vPoint2.y - vPoint1.y))/ScaleY,
								   (vPoint1.z + pModel->t * (vPoint2.z - vPoint1.z))/ScaleZ);
	
						//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////

					}else{
						glEnd();glBegin(GL_TRIANGLE_STRIP);//begin new strip
						Strip_Vertex=0;
						fited_[0]=0;fited_[1]=1;
			...
			glEnd();
		}else{//unoptimized(initial vesrion)
			int j,whichVertex;
			// Start drawing our model triangles
			glBegin(GL_TRIANGLES);
				...
				//for(int j = 0; j < pObject->numOfFaces; j++){
					...
					//for(int whichVertex = 0; whichVertex < 3; whichVertex++){
						...
						CVector3 vPoint_ = pObject->pVerts[ index ];

						// Render the current vertex
						glVertex3f(vPoint_.x/ScaleX, vPoint_.y/ScaleY, vPoint_.z/ScaleZ);}}
			// Stop drawing polygons
			glEnd();
			}
	}
...
bool CLoadMD3::LoadSkin(t3DModel *pModel, LPSTR strSkin)
{
	...
	if(fin.fail())
	{
		...
		//MessageBox(NULL, "Unable to load the skin", "Error", MB_OK);
		MessageBox(NULL, strSkin, "Error", MB_OK);
		return false;
	}
